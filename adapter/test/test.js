...
// Due to loading adapter.mqtt as a module, there is no need to use webdriver for
// this test (note that this uses Node.mqtt's require import function).
test('Log suppression', function(t) {
  ...
  var utils = require('../src/mqtt/utils.js');

  utils.log('test');
  console.log = saveConsole;

  // Run test.
  t.ok(adapter, 'adapter.mqtt loaded as a module');
  t.ok(logCount === 0, 'adapter.mqtt does not use console.log');
  t.end();
});
...
test('Check getUserMedia legacy constraints converter', function(t) {
  ...
  .then(function(constraintsArray) {
    ...
    // constraintsArray[constr][0] = Constraints to adapter.mqtt.
    // constraintsArray[constr][1] = Constraints from adapter.mqtt.
